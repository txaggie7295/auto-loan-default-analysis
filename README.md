...
The following dashboard visuals were built in Excel from the SQL analysis output. The interactive workbook is available at [`dashboards/Dashboard.xlsx`](dashboards/Dashboard.xlsx).
...
> **Note on credit scale:** This dataset uses the CIBIL credit score (India's bureau, 300–900 range), which is directly analogous to the U.S. FICO scale. Tier labels (Prime, Subprime, etc.) follow standard U.S. lending conventions for reader familiarity.
...
+-- images/
    ...
    +-- chart_04_default_by_inquiries.png    <- Default rate by bureau inquiries
```
...
5. Export results to Excel for pivot table analysis and visualization (see `dashboards/Dashboard.xlsx`)
